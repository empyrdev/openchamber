...
export function createOpenCodeManager(context: vscode.ExtensionContext): OpenCodeManager {
  let server: Awaited<ReturnType<typeof spawnManagedOpenCodeServer>> | null = null;
  ...
  async function startInternal(
    ...
    try {
      ...
      startup.signal.throwIfAborted();
      const startedServer = await spawnManagedOpenCodeServer(serverCwd, port, READY_CHECK_TIMEOUT_MS, startup.signal);
      server = startedServer;
      await startedServer.ready;

      if (startedServer.url) {
        // Validate readiness for the current workspace context.
        const ready = await waitForReady(startedServer.url, READY_CHECK_TIMEOUT_MS, getOpenCodeAuthHeaders(), startup.signal);
        startup.signal.throwIfAborted();
