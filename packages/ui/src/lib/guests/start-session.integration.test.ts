...
const created: Session = { id: 'background-session', directory: '/project-b', title: 'Task', projectID: 'b', cost: 0, tokens: { input: 0, output: 0, reasoning: 0, cache: { read: 0, write: 0 } }, time: { created: 1, updated: 1 } };
...
test('a replaced SDK client cannot publish a late creation even when the runtime key is unchanged', async () => {
  const create = setup();
  let client = OpenCode.make({ baseUrl: 'http://sync.test' });
  spyOn(opencodeClient, 'getSdkClient').mockImplementation(() => client);
  create.mockImplementation(async () => {
    client = OpenCode.make({ baseUrl: 'http://replacement.test' });
