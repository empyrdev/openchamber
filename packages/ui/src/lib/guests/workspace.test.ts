...
const session = (id: string, directory: string): Session => ({ id, directory, title: id, projectID: 'upstream', cost: 0, tokens: { input: 0, output: 0, reasoning: 0, cache: { read: 0, write: 0 } }, time: { created: 1, updated: 1 } });
let manager: ChildStoreManager;
beforeEach(() => {
  manager = new ChildStoreManager();
  setSyncRefs(OpenCode.make({ baseUrl: 'http://sync.test' }), manager, '/a');
...
describe('extension workspace projection', () => {
  ...
  test('blocking requests and retries expose state without conversation data', () => {
    ...
    child.setState({ permission: { 'b-session': [{ id: 'permission', sessionID: 'b-session', action: 'bash', resources: [], metadata: {} }] } });
    expect(readB().sessions[0].activity).toBe('waiting-permission');
    child.setState({ permission: {}, form: { 'b-session': [{ id: 'form', sessionID: 'b-session', title: 'Question', fields: [{ key: 'answer', type: 'boolean' }] }] } });
