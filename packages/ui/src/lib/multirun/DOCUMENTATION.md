...
`createSession.ts` allocates OpenCode's canonical session ID before creation and
sends that ID both as the create request ID and in membership metadata. Callers
register and dispatch only after the response confirms the exact ID-bound marker.
This is one create operation, so it needs no metadata update endpoint or
concurrent metadata-editor protocol.

A failed create-response validation attempts to delete only that newly created session. Worktrees
