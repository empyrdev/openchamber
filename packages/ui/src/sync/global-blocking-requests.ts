...
// `session.deleted`) and seeded once from the host. Nothing
// streams through here, so consumers subscribe per session ID without cost.

export type PendingBlockingRequests = {
  directory: string;
  // The global host seed exists for tray badges. Full request records stay in
  // directory stores, which own rendering and answering their fields.
...
export const applyGlobalBlockingRequestEvents = (rawDirectory: string, payloads: readonly SyncEvent[]): void => {
  ...
  for (const payload of payloads) {
    switch (payload.type) {
      ...
      case 'form.created': {
        const request = payload.properties.form;
