...
function setup(surface: "desktop" | "mobile" | "vscode" = "desktop", recordsFor = transcript) {
  ...
  const sdk = { getSessionMessages: async (sessionID: string, options?: { limit?: number; cursor?: string }) => {
    const url = new URL(`http://cache.test/session/${sessionID}/message`)
    ...
    return { items: page, cursor: cursor ? { next: cursor } : {} }
  } }
...
describe("session cache retention", () => {
  ...
  test("busy and blocking background sessions outlive the idle grace until they settle", async () => {
    ...
    store.setState({ form: { a: [{ id: "form", sessionID: "a", title: "Question", fields: [{ key: "answer", type: "boolean" }] }] } })
