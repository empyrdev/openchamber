...
describe("SessionMessageLoader", () => {
  ...
  test("keeps duplicate session IDs scoped to their own directory while initial history expands", async () => {
    ...
    expect(calls).toEqual([
      { directory: providerDirectory, cursor: undefined },
      { directory: selectedDirectory, cursor: undefined },
      { directory: providerDirectory, cursor: `${providerDirectory}-cursor` },
      { directory: selectedDirectory, cursor: `${selectedDirectory}-cursor` },
    ])
    loader.dispose()
