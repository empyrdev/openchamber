...
const PREFETCH_MESSAGE_PAGE_SIZE = 50
...
const hasInitialTurns = (messages: Message[]): boolean => messages.filter(isUserMessage).length >= INITIAL_USER_TURNS
...
export class SessionMessageLoader {
  ...
  private evictSessionHistory(target: SessionMessageTarget): void {
    ...
    const draft = {
      ...
      permission: { ...current.permission }, form: { ...current.form },
    }
  ...
  initializeCreatedSession(target: SessionMessageTarget): void {
    ...
    if (options?.reason === "prefetch" && !entry.snapshot.resolved) {
      this.patchEntry(entry, { limit: Math.min(entry.snapshot.limit, PREFETCH_MESSAGE_PAGE_SIZE) })
    }
    ...
    return this.startLoad(normalized, entry, store, kind, async (isCurrent, performance) => {
      await this.loadInitial(
        normalized, entry, store, isCurrent, performance,
        options?.reason === "prefetch" ? PREFETCH_MESSAGE_PAGE_SIZE : getInitialPageSize(),
      )
    })
  }
  ...
  private async loadInitial(
    ...
    const firstLimit = Math.max(entry.snapshot.limit, storeMessageCount, initialPageSize)
