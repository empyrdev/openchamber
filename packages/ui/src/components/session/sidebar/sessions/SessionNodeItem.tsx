...
function SessionNodeItemComponent(props: SessionNodeItemProps): React.ReactNode {
  ...
  const rowBadges = (pendingPermissionCount > 0 || pendingFormCount > 0) ? (
    <>
      ...
      {pendingFormCount > 0 ? (
        <span className="inline-flex flex-shrink-0 items-center gap-1 rounded bg-status-info/10 px-1 py-0.5 text-[0.7rem] text-status-info" title={t('sessions.sidebar.session.status.questionPendingMany', { count: pendingFormCount })} aria-label={t('sessions.sidebar.session.status.questionPendingMany', { count: pendingFormCount })}>
