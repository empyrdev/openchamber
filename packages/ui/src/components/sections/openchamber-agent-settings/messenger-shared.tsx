...
/**
 * Listener settings are authoritative for whether the full messenger
 * integration is enabled. Older saved configurations omit the field and
 * retain the server's default-on behavior.
 */
// eslint-disable-next-line react-refresh/only-export-components
export function isMessengerIntegrationEnabled(
  connection: Pick<
    MessengerConnection,
    'type' | 'discordListenerEnabled' | 'telegramListenerEnabled'
  >,
): boolean {
  return connection.type === 'discord'
    ? connection.discordListenerEnabled !== false
    : connection.telegramListenerEnabled !== false;
}
