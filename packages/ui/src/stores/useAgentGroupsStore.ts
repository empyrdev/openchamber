...
const buildAgentGroups = (
  ...
  worktrees: ReadonlyMap<string, WorktreeMetadata>,
  projectDirectory: string,
): AgentGroup[] => {
  const groups = new Map<string, AgentGroup>()
  for (const session of sessions) {
    const path = normalize(session.directory)
    const identity = getMultiRunIdentity(session, projectDirectory)
    if (!path || !identity || identity.role !== 'run') continue
    const entry: AgentGroupSession = {
      id: session.id,
      path,
      providerId: identity.providerID,
      modelId: identity.modelID,
      instanceNumber: identity.index ?? 1,
      displayLabel: session.title || identity.modelID,
      branch: worktrees.get(path)?.branch,
      worktreeMetadata: worktrees.get(path),
    ...
    if (existing) {
      ...
      existing.lastActive = Math.max(existing.lastActive, session.time.updated)
      continue
    }
    groups.set(identity.key, {
      id: identity.key,
      name: identity.groupSlug,
      sessions: [entry],
      sessionCount: 1,
      lastActive: session.time.updated,
    })
  }
  return [...groups.values()].sort((left, right) => right.lastActive - left.lastActive)
}
    ...
    deleteGroupSessions: async (sessions, options) => {
      ...
      if (removeWorktrees) {
        ...
        for (const [path, pathSessions] of worktreesByPath) {
          ...
          try {
            const remaining = await listGlobalSessionPages((options) => opencodeClient.listSessionsPage(options), {
              directory: path, pageSize: 500,
            });
            assertCurrent();
