...
export const createSessionRuntime = ({ writeSseEvent, getNotificationClients, broadcastEvent }) => {
  ...
  // Pending permission and form requests per session, kept from the same
  ...
  const processBlockingRequestPayload = (payload) => {
    ...
    const properties = payload.properties && typeof payload.properties === 'object' ? payload.properties : {};
    if (payload.type === 'permission.asked' || payload.type === 'form.created') {
      const request = payload.type === 'form.created' && properties.form && typeof properties.form === 'object'
        ? properties.form : properties;
      const sessionId = readRequestId(request.sessionID);
      const requestId = readRequestId(request.id);
      if (!sessionId || !requestId) return;
      getOrCreatePendingRequests(sessionId)[payload.type === 'permission.asked' ? 'permissions' : 'forms'].set(requestId, request);
      return;
    }
